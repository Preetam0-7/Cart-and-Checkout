...
class CheckoutServiceImplTest {
    ...
    void testPlaceOrder_SuccessfulFlow() {
        ...
        InventoryAvailabilityResponseDTO inventoryDTO =
                new InventoryAvailabilityResponseDTO("SKU-RED-M-001", 10, 1001);
        ResponseEntity<InventoryAvailabilityResponseDTO[]> inventoryResponse =
                new ResponseEntity<>(new InventoryAvailabilityResponseDTO[]{inventoryDTO}, HttpStatus.OK);
        when(restTemplate.postForEntity(
                anyString(), any(), eq(InventoryAvailabilityResponseDTO[].class))
        ).thenReturn(inventoryResponse);

        // Mock OMS response with valid orderItemIds
        Map<String, String> orderItemMap = new HashMap<>();
        orderItemMap.put("SKU-RED-M-001", "ORD123");
        OrderResponseDTO orderResponse = new OrderResponseDTO();
        orderResponse.setOrderItemIds(orderItemMap);

        when(restTemplate.postForObject(anyString(), any(), eq(OrderResponseDTO.class)))
                .thenReturn(orderResponse);

        // Mock inventory update exchange
    ...
    @Test
    void testUpdateInventoryStock_Failure() {
        when(shoppingCartRepository.findByUserId(101)).thenReturn(Optional.of(cart));
        when(cartItemRepository.findByCart(cart)).thenReturn(cartItems);
        ...
        orderItemIds.put("SKU-RED-M-001", "ORD123");

        OrderResponseDTO orderResponse = new OrderResponseDTO();
        orderResponse.setOrderItemIds(orderItemIds);
        when(restTemplate.postForObject(anyString(), any(), eq(OrderResponseDTO.class)))
                .thenReturn(orderResponse);

        // Simulate failure in inventory update
    ...
    @Test
    void testPlaceOrderInOMS_ReturnsEmptyOrderItemIds() {
        when(shoppingCartRepository.findByUserId(101)).thenReturn(Optional.of(cart));
        when(cartItemRepository.findByCart(cart)).thenReturn(cartItems);
        ...
        OrderResponseDTO orderResponse = new OrderResponseDTO(); // No orderItemIds set
        when(restTemplate.postForObject(anyString(), any(), eq(OrderResponseDTO.class)))
                .thenReturn(orderResponse);
